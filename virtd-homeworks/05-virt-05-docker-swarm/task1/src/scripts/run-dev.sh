...
if [[ ! -f "$FUNCTIONS_FILE" ]]; then
    echo "$(colorize "ERROR" "[ERROR] Файл '$FUNCTIONS_FILE' не найден. Убедитесь, что он находится в той же директории, что и скрипт.")"
    exit 1
fi
...
# Заголовок
echo "$(colorize "STEP" "[START] Режим: ПРЕДПРОД (dev)")"
...
echo "$(colorize "INFO" "[INFO] Проверяем наличие подсети с именем '${SUBNET_NAME}'...")"
...
if echo "$EXISTING_SUBNETS" | grep -q "^$SUBNET_NAME "; then
    ...
    echo "$(colorize "SUCCESS" "[INFO] Подсеть с именем '${SUBNET_NAME}' не найдена. Будет использовано имя '${NEW_SUBNET_NAME}'.")"
fi
...
if echo "$EXISTING_SUBNETS" | grep -q "^$NEW_SUBNET_NAME "; then
    ...
    echo "$(colorize "SUCCESS" "[INFO] Имя '${NEW_SUBNET_NAME}' свободно для использования.")"
fi
...
echo "$(colorize "INFO" "[INFO] Проверяем наличие образа с именем '${IMAGE_NAME}'...")"
...
if echo "$EXISTING_IMAGES" | grep -q "^$IMAGE_NAME\$"; then
    ...
    echo "$(colorize "SUCCESS" "[INFO] Образ с именем '${IMAGE_NAME}' не найден. Будет использовано имя '${NEW_IMAGE_NAME}'.")"
fi
...
if echo "$EXISTING_IMAGES" | grep -q "^$NEW_IMAGE_NAME\$"; then
    ...
    echo "$(colorize "SUCCESS" "[INFO] Имя '${NEW_IMAGE_NAME}' свободно для использования.")"
fi
...
# Завершение
if [ $? -eq 0 ]; then
    echo "$(colorize "SUCCESS" "[SUCCESS] Все этапы успешно завершены.")"
else
